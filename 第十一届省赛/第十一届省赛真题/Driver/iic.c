...
/**
 * @brief   PCF8591 AD转换(模拟量 -> 数字量)
 * @param   addr  通道地址：
 *                0x41 = 光敏电阻(通道1)
 *                0x43 = 滑动变阻器(通道3)
 *                0x40 = 外部输入(通道0)
 *                0x42 = 差分输入(通道2)
 * @return  8位AD值(0~255，对应0~5V模拟量)
 * @note    PCF8591 I2C地址：写=0x90, 读=0x91
 *          返回的是上一次转换的结果(流水线方式)
 */
unsigned char Ad_Read(unsigned char addr)
{
	...
	I2CWaitAck();
	I2CSendByte(addr);         // 写入通道控制字
	I2CWaitAck();

	I2CStart();                // 重复起始条件，切换为读操作
	I2CSendByte(0x91);         // 选中PCF8591，读操作
	I2CWaitAck();
	temp = I2CReceiveByte();   // 读取AD转换结果

	I2CSendAck(1);             // 发送NACK，不再继续读取
	I2CStop();
...
/**
 * @brief   AT24C02 EEPROM连续写入
 * @param   str   待写入的数据数组指针
 * @param   addr  EEPROM起始地址(0~255)
 * @param   num   写入字节数
 * @note    AT24C02 I2C地址：写=0xA0, 读=0xA1
 *          页写入模式(8字节/页)，跨页需注意地址对齐
 *          写入后需要较长延时等待EEPROM内部编程完成(约5~10ms)
 */
void EEPROM_Write(unsigned char *str, unsigned char addr, unsigned num)
{
	...
	while(num--)
	{
		I2CSendByte(*str++);   // 逐字节写入数据
		I2CWaitAck();
		I2C_Delay(200);        // 字节间延时，保证写入完成
	}
	I2CStop();

	/* 写入完成后的长延时，等待EEPROM内部编程 */
	I2C_Delay(255);
	I2C_Delay(255);
	I2C_Delay(255);
	I2C_Delay(255);
...
/**
 * @brief   AT24C02 EEPROM连续读取
 * @param   str   读取数据存放的数组指针
 * @param   addr  EEPROM起始地址(0~255)
 * @param   num   读取字节数
 * @note    读取流程：先写入起始地址(伪写操作)，再切换为读模式连续读取
 *          最后一个字节发送NACK结束读取，其余字节发送ACK继续
 *          读取期间关闭中断EA，防止I2C时序被打断
 */
void EEPROM_Read(unsigned char *str, unsigned char addr, unsigned num)
{
	I2CStart();
	I2CSendByte(0xa0);         // 选中AT24C02，写操作(设置读取起始地址)
	I2CWaitAck();
	I2CSendByte(addr);         // 写入起始地址
	I2CWaitAck();
	...
	while(num--)
	{
		*str++ = I2CReceiveByte();
		if(num)
			I2CSendAck(0);     // 还有数据要读，发送ACK
		else
			I2CSendAck(1);     // 最后一个字节，发送NACK结束
	}
	I2CStop();
	EA = 1;                    // 恢复中断
}
